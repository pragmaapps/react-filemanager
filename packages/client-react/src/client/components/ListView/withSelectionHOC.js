...
export default class WithSelection extends PureComponent {
  ...
  handleRowClick = ({ event, rowData, ...args }) => {
    const { items, idPropName, selection, compressor } = this.props;
    ...
    if (event.ctrlKey && !compressor || event.metaKey && !compressor) { // metaKey is for handling "Command" key on MacOS
