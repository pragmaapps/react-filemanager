      ...
      onSubmit: async (name) => {
        ...
        try {
          const resourceChildren = await api.getChildrenForId(
            apiOptions, { id: selectedResources[0].path, sortBy: null, sortDirection: null, where: apiOptions.fromWhere }
          );
          ...
          if (alreadyExists) {
            ...
            hideDialog();
            const result = await api.renameResource(apiOptions, selectedResources[0].path, name, selectedResources[0].name);
